...
async def create_room(
    ...
    await _write_audit(current_user["id"], current_user["email"], "room_create", f"Created room {body.name}")
...
async def delete_room(room_id: str, current_user: dict = Depends(get_admin_user)):
    ...
    await _write_audit(current_user["id"], current_user["email"], "room_delete", f"Archived room {room_id}")
...
async def join_room(room_id: str, current_user: dict = Depends(get_current_user)):
    ...
    await _write_audit(current_user["id"], current_user["email"], "room_join", f"Joined room {room_id}")
    return {"ok": True}
...
async def leave_room(room_id: str, current_user: dict = Depends(get_current_user)):
    ...
    await _write_audit(current_user["id"], current_user["email"], "room_leave", f"Left room {room_id}")
    return {"ok": True}
...
async def update_member_role(
    ...
    await _check_supervisor_or_admin(room_id, current_user)
    await rooms_col().update_one(
        {"_id": str_to_oid(room_id), "members.user_id": user_id},
        {"$set": {"members.$.room_role": body.room_role}}
    )
    await _write_audit(current_user["id"], current_user["email"], "role_update", f"Updated {user_id} role in {room_id}")
    return {"ok": True}
...
async def kick_member(
    ...
    await _write_audit(current_user["id"], current_user["email"], "member_kick", f"Kicked {user_id} from {room_id}")
...
async def mute_member(
    ...
    await _write_audit(current_user["id"], current_user["email"], "member_mute", f"Muted {user_id} in {room_id}")
