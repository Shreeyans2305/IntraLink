  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
      <div className="mb-2 flex items-start justify-between gap-3">
        <h4 className="text-sm font-semibold text-zinc-200">📊 {poll.question}</h4>
        <div className="flex flex-wrap gap-2 text-[11px] text-zinc-400">
          {poll.anonymous ? <span className="rounded-full bg-zinc-800 px-2 py-1">Anonymous</span> : null}
          {poll.closeAt ? (
      ...
      {poll.closed ? <p className="mt-2 text-xs text-zinc-400">Poll closed</p> : null}
    </div>
  )
}
